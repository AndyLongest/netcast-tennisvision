...
模型不保存在 Git 仓库中。四个生产模型合计约 94.2 MB，首次安装时由脚本下载或重建，
...
| `models/osnet_ain_x1_0_msmt17_256x128_amsgrad_ep50_lr0.0015_coslr_b64_fb10_softmax_labsmth_flip_jitter.pth` | 17.3 MB | 从固定提交下载 Torchreid OSNet-AIN 球员重识别权重 |
...
在另一台已经安装成功的电脑上，把下面四个文件复制到一个普通文件夹：
...
Ultralytics 权重及软件需要按 AGPL-3.0 或相应 Enterprise License 使用；落点参考数据和
OSNet-AIN 权重训练数据的再分发条件仍需在公开发布前复核。详见 `THIRD_PARTY_NOTICES.md` 和
